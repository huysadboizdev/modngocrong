...
public class ServerListScreen : mScreen, IActionListener
{
    ...
    public static string ipEncrypt = "1(N)z$1I7m_v@zqWXXwGSM{U=4N+VHltQ+v=#dmjw8E=|g(;NC{+SqAF@7aNg)Z8JMgz|ExI}xge@efj`;Fcpp9gt3@F85(^IIl6";
	public static int ipEncryptKey = 261005;
	public static string ListIP = "NRO VEGETA:160.22.106.46:14445:0,0,0";

	public static string linkDefault = ListIP;
		...
		for (int i = 0; i < cmd.Length; i++)
		{
			switch (i)
			{
				...
				case 3:
					...
					{
						cmd[3] = new Command(mResources.data_type_mod, this, 1234, null);
					}
					break;
				...
				case 4:
					if (nCmdPlay == 1)
					{
						cmd[4] = new Command(mResources.data_type_mod, this, 1234, null);
					}
					else
					{
	...
	public static void LoadIP()
	{
		try
		{
			// The endpoint bundled with this build must not be replaced by stale RMS data.
			if (!string.IsNullOrEmpty(ListIP))
			{
				linkDefault = ListIP;
				GetServerList(ListIP);
				return;
			}
	...
	public override void switchToMe()
	{
		...
		// cập nhật caption cho các nút bằng cách tìm theo idAction để tránh phụ thuộc index
		if (cmd != null)
		{
			// server label (vẫn dùng công thức cũ)
			int serverIndex = 2 + nCmdPlay;
			if (serverIndex >= 0 && serverIndex < cmd.Length && cmd[serverIndex] != null)
			{
				cmd[serverIndex].caption = mResources.server + ": " + nameServer[ipSelect];
			}

			// change_account label
			int changeIndex = 1 + nCmdPlay;
			if (changeIndex >= 0 && changeIndex < cmd.Length && cmd[changeIndex] != null)
			{
				cmd[changeIndex].caption = mResources.change_account;
			}

			// cập nhật caption nút DataType nếu có
			for (int i = 0; i < cmd.Length; i++)
			{
				if (cmd[i] == null) continue;
				if (cmd[i].idAction == 1234)
				{
					cmd[i].caption = mResources.data_type_mod + " (" + ModFunc.GetDataTypeString(ModFunc.currentDataType) + ")";
				}
				else if (cmd[i].idAction == 8)
				{
					cmd[i].caption = mResources.option;
				}
			}
		}
	...
	public void perform(int idAction, object p)
	{
		switch (idAction)
		{
			...
			case 1234:
				{
					...
					string current = ModFunc.GetDataTypeString(ModFunc.currentDataType);

					vector.addElement(new Command((current == "Int" ? "[Đang dùng] " : "") + "Int", this, 12345, ModFunc.DataType.Int));
					vector.addElement(new Command((current == "Double" ? "[Đang dùng] " : "") + "Double", this, 12345, ModFunc.DataType.Double));
					vector.addElement(new Command((current == "Long" ? "[Đang dùng] " : "") + "Long", this, 12345, ModFunc.DataType.Long));
			...
			case 12345:
				{
					ModFunc.DataType selectedType = (ModFunc.DataType)p;
					ModFunc.SetDataType(selectedType);

					for (int j = 0; j < cmd.Length; j++)
					{
						if (cmd[j] != null && cmd[j].idAction == 1234)
						{
							cmd[j].caption = mResources.data_type_mod + " (" +
											 ModFunc.GetDataTypeString(ModFunc.currentDataType) + ")";
							break;
						}
					}

					GameScr.info1.addInfo("Đã chuyển sang kiểu dữ liệu: " +
										  ModFunc.GetDataTypeString(ModFunc.currentDataType), 0);
					return;
				}
			case 11:
				{
