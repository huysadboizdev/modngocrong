...
public class Panel : IActionListener, IChatable
{
	...
	public void addItemDetail(Item item)
	{
		try
		{
			...
			if (item.itemOption != null)
			{
				...
				{
					if (item.itemOption[i] == null) continue;
			...
			// ===== Màu tên theo option 41 =====
			bool flag = false;
			bool showId = (ModFunc.isInventory && ModFunc.isShowID);
			string itemName = showId
				? "[" + item.template.id + "] " + item.template.name + text2
				: item.template.name + text2;

			if (item.itemOption != null)
			{
				for (int j = 0; j < item.itemOption.Length; j++)
				{
					if (item.itemOption[j] == null) continue;
					if (item.itemOption[j].optionTemplate.id == 41)
					{
						flag = true;
						int param = item.itemOption[j].param;
						if (param == 1)
							text += "|0|1|" + itemName;
						else if (param == 2)
							text += "|2|1|" + itemName;
			...
			if (item.itemOption != null)
			{
				...
				{
					if (item.itemOption[k] == null || item.itemOption[k].optionTemplate == null)
						continue;
					int id = item.itemOption[k].optionTemplate.id;
					if (id == 72)
						continue;
					...
					if (id == 228)
					{
						int sao = item.itemOption[k].param;
						if (sao > 0)
						{
							text += "\n|1|1|" + sao + " Sao Cường Hóa";
						}
						continue;
					}

					// Các option khác (kể cả name bắt đầu $)
					if (item.itemOption[k].optionTemplate.name.StartsWith("$"))
					{
						empty = item.itemOption[k].getOptiongColor();
						if (item.itemOption[k].param == 1)
							text += "\n|1|1|" + empty;
						else if (item.itemOption[k].param == 0)
							text += "\n|0|1|" + empty;
					}
					else
					{
		...
		{
			Res.outz("addItemDetail ex: " + ex.StackTrace);
		}
	}
	...
	private void setTabPetInventory(bool isPet2)
	{
		...
		if (pet == null)
		{
			Debug.LogWarning("[Panel] Pet null -> Không thể mở tab thú cưng");
			currentListLength = 0;
			cmyLim = 0;
			cmy = cmtoY = 0;
			selected = -1;
		...
		if (pet.arrItemBody == null)
		{
			Debug.LogWarning("[Panel] pet.arrItemBody null -> tạo mảng trống");
			pet.arrItemBody = new Item[0];
		}
		...
		if (cmyLast == null || currentTabIndex >= cmyLast.Length)
		{
			cmyLast = new int[22];
		}

		cmy = cmtoY = cmyLast[currentTabIndex];
		if (cmy < 0 || cmy > cmyLim) cmy = cmtoY = 0;
		selected = GameCanvas.isTouch ? -1 : 0;
	}
			...
			for (int i = 0; i < num; i++)
			{
				...
				if (item != null)
				{
					...
					if (item.buySpec > 0)
					{
						...
						mFont.tahoma_7b_dark.drawString(g, Res.formatNumber(item.buySpec), num2 + num4 - 17, priceTextY, mFont.RIGHT);
					}
					if (item.buyCoin != 0 || item.buyGold != 0 || item.buyRuby != 0)
					{
						if (typeShop != 2 && item.powerRequire == 0L)
						{
							if (item.buyCoin > 0 && item.buyGold > 0)
							{
								if (item.buyCoin > 0)
								{
									...
									mFont.tahoma_7b_yellow.drawString(g, Res.formatNumber(item.buyCoin), num2 + num4 - 17, priceTextY, mFont.RIGHT);
								}
								if (item.buyGold > 0)
								{
									g.drawImage(imgLuong, num2 + num4 - 7, num3 + h - 5, 3);
									mFont.tahoma_7b_blue.drawString(g, Res.formatNumber(item.buyGold), num2 + num4 - 17, priceTextY, mFont.RIGHT);
								}
							}
							else
							{
							...
							if (item.buyRuby > 0)
							{
								...
								mFont.tahoma_7b_red.drawString(g, Res.formatNumber(item.buyRuby), num2 + num4 - 17, priceTextY, mFont.RIGHT);
							}
						}
						if (typeShop == 2 && currentTabIndex <= 3 && !Equals(GameCanvas.panel2))
						{
							if (item.buyCoin > 0 && item.buyGold > 0)
							{
								...
								{
									g.drawImage(imgLuongKhoa, num2 + num4 - 7, num3 + h - 5, 3);
									mFont.tahoma_7b_yellow.drawString(g, Res.formatNumber2(item.buyCoin), num2 + num4 - 17, priceTextY, mFont.RIGHT);
								}
								if (item.buyGold > 0)
								{
									g.drawImage(imgLuong, num2 + num4 - 7, num3 + h - 5, 3);
									mFont.tahoma_7b_green.drawString(g, Res.formatNumber2(item.buyGold), num2 + num4 - 17, priceTextY, mFont.RIGHT);
								}
							}
							else
							{
				...
				if (item.buyType == 1)
				{
					...
					{
						g.drawImage(imgLuongKhoa, num2 + num4 - 7, num3 + h - 5, 3);
						mFont.tahoma_7b_red.drawString(g, Res.formatNumber2(item.buyGold), num2 + num4 - 17, priceTextY, mFont.RIGHT);
					}
				}
	...
	private void paintInventory(mGraphics g)
	{
		...
		// Null safety check
		if (arrItemBody == null || arrItemBag == null)
		{
			return;
		}
		// Tính toán chiều dài list
		currentListLength = checkCurrentListLength(arrItemBody.Length + arrItemBag.Length / 6);
		TAB_W_NEW = 1;

		// Set clip và translate
		g.setClip(xScroll, yScroll, wScroll, hScroll);
		g.translate(0, -cmy);

		try
		{
			// Vẽ items trên người (body)
			int bodyStartY = yScroll;
			PaintBodyItems(g, arrItemBody, bodyStartY);

			// Vẽ items trong túi (bag)
			int bagStartY = bodyStartY + arrItemBody.Length * ITEM_HEIGHT;
			PaintBagItems(g, arrItemBag, bagStartY, arrItemBody);
		}
		catch (Exception ex)
		{
			// Log lỗi nếu cần
			Debug.LogError($"Error in paintInventory: {ex.Message}");
		}
	...
	/// <summary>
	/// Vẽ items đang mặc trên người
	/// </summary>
	private void PaintBodyItems(mGraphics g, Item[] arrItemBody, int bodyStartY)
	{
		for (int j = 0; j < arrItemBody.Length; j++)
		{
			...
			int iconX = xScroll;
			int iconY = y;
			int iconW = ITEM_HEIGHT - 1;
			int iconH = ITEM_HEIGHT - 1;

			// Skip nếu nằm ngoài vùng hiển thị
			...
			// Kiểm tra selected
			GetInventorySelect_isbody(j, newSelected, arrItemBody);
			bool isSelected = (j == selected);

			// Vẽ background
			PaintItemBackground(g, item, x, y, wScroll, ITEM_HEIGHT - 1, iconX, iconY, iconW, iconH, isSelected, j);
			...
			// Vẽ tên item với màu phù hợp
			mFont nameFont = GetItemNameFont(item);
			string itemName = GetItemDisplayName(item);
			nameFont.drawString(g, itemName, x + 5, y + 1, 0);

			// Vẽ thông số item
			PaintItemOptions(g, item, x + 5, y + 10);

			// Vẽ icon item
			SmallImage.drawSmallImage(g, item.template.iconID, iconX + iconW / 2, iconY + iconH / 2, 0, 3);

			// Vẽ các hiệu ứng đặc biệt
			PaintItemSpecialEffects(g, item, iconX, iconY, iconW, iconH);
		}
	}

	/// <summary>
	/// Vẽ items trong túi đồ (dạng lưới)
	/// </summary>
	private void PaintBagItems(mGraphics g, Item[] arrItemBag, int bagStartY, Item[] arrItemBody)
	{
		const int CELL_SIZE = 28;     // chinh frame khung
		const int GRID_COLUMNS = 6;   // 6 cột
		...
		for (int i = 0; i < arrItemBag.Length; i++)
		{
			...
			int x = xScroll + col * (CELL_SIZE + 1);
			int y = bagStartY + row * ITEM_HEIGHT;
			int cellW = CELL_SIZE;
			int cellH = ITEM_HEIGHT - 1;

			// Nếu ô nằm ngoài vùng hiển thị thì bỏ qua
			if (y - cmy > yScroll + hScroll || y - cmy < yScroll - ITEM_HEIGHT)
				continue;
			...
			int inventorySelectBag = GetInventorySelect_bag(selected, newSelected, arrItemBody);
			bool isSelected = (i == inventorySelectBag);

			// Viền chọn
			if (isSelected)
			{
				g.setColor(16711680); // Đỏ
				g.drawRect(x - 1, y - 1, cellW + 1, ITEM_HEIGHT);
			}

			// Nền ô (item hoặc trống)
			if (item != null)
			{
				PaintBagItemBackground(g, item, x, y, cellW, cellH, isSelected);
			}
			else
			{
				g.setColor(6047789, 0.5f);
				g.fillRect(x, y, cellW, cellH, 5);
			}

			// Nếu không có item thì bỏ qua
			if (item == null)
				continue;

			// Overlay khi item được chọn trong panel combine
			...
			PaintItemSpecialEffects(g, item, x, y, cellW, cellH);
			...
			if (item.quantity > 1)
			{
				mFont.tahoma_7b_white.drawString(g,
					...
					x + cellW,
					y + cellH - mFont.tahoma_7_yellow.getHeight(),
					1);
			}
		}
	}
	...
	/// <summary>
	/// Vẽ background cho item với màu theo upgrade level
	/// </summary>
	private void PaintItemBackground(mGraphics g, Item item, int x, int y, int w, int h,
		int iconX, int iconY, int iconW, int iconH, bool isSelected, int index)
	{
		...
		// Kiểm tra upgrade level để đổi màu
		if (item != null && item.itemOption != null)
		{
			foreach (var option in item.itemOption)
			{
				...
				{
					byte colorId = (byte)GetColor_Item_Upgrade(option.param);
					int bgColor = GetColor_ItemBg(colorId);
		...
		// Vẽ icon background
		g.setColor(6047789, 0.5f);
		g.fillRect(iconX, iconY, iconW, iconH, 5);

		// Vẽ effect item
		paintEffectItem(g, item, iconX, iconY);

		// Vẽ overlay nếu item được select trong panel
		if (item != null && item.isSelect && GameCanvas.panel?.type == 12)
		{
			g.setColor(isSelected ? 7040779 : 6047789);
			g.fillRect(iconX, iconY, iconW, iconH, 5);
	...
	/// <summary>
	/// Vẽ background cho item trong túi (bag)
	/// </summary>
	private void PaintBagItemBackground(mGraphics g, Item item, int x, int y, int w, int h, bool isSelected)
	{
		g.setColor(6047789, 0.5f);
		g.fillRect(x, y, w, h, 5);

		// Kiểm tra upgrade level
		if (item.itemOption != null)
		{
			foreach (var option in item.itemOption)
			{
		...
		// Kiểm tra auto items (màu xanh lá)
		if (ModFunc.GI()?.listItemAuto?.Any(itemAuto =>
			item.template.id == itemAuto.id && item.template.iconID == itemAuto.iconID) == true)
		{
			g.setColor(65280); // Xanh lá
			g.fillRect(x, y, w, h, 5);
		}

		// Kiểm tra filter items (màu đỏ)
		if (ModFunc.listFilterItems?.Any(filterItem => filterItem.id == item.template.id) == true)
		{
			g.setColor(16711680); // Đỏ
			g.fillRect(x, y, w, h, 5);
		}
	...
	/// <summary>
	/// Lấy font cho tên item theo upgrade level
	/// </summary>
	private mFont GetItemNameFont(Item item)
	{
		if (item?.itemOption == null)
		{
			return mFont.tahoma_7_green2;
		}
	...
	/// <summary>
	/// Lấy tên hiển thị của item (có hoặc không có ID)
	/// </summary>
	private string GetItemDisplayName(Item item)
	{
		...
		if (ModFunc.isShowID)
		{
			return $"[{item.template.id}] {item.template.name}";
		}
	...
	/// <summary>
	/// Vẽ các option/thông số của item
	/// </summary>
	private void PaintItemOptions(mGraphics g, Item item, int x, int y)
	{
		...
		// Lấy option đầu tiên
		if (item.itemOption[0] != null && item.itemOption[0].IsValidOption())
		{
			optionsText = item.itemOption[0].getOptionString();
		}

		// Lấy các option tiếp theo (tối đa 3 option)
		if (item.itemOption.Length > 1)
		{
			int maxOptions = System.Math.Min(item.itemOption.Length, 3);

			for (int i = 1; i < maxOptions; i++)
			{
				if (item.itemOption[i] != null && item.itemOption[i].IsValidOption())
				{
					optionsText += ", " + item.itemOption[i].getOptionString();
				}
			}
		}

		// Chọn màu font
		mFont optionFont = mFont.tahoma_7_blue;

		if (item.compare < 0 && item.template.type != 5)
		{
			optionFont = mFont.tahoma_7_red;
		}

		// Vẽ text
		optionFont.drawString(g, optionsText, x, y, mFont.LEFT);
	}

	/// <summary>
	/// Vẽ các hiệu ứng đặc biệt của item
	/// (được tắt trong Inventory để tránh lệch icon và rối giao diện)
	/// </summary>
	private void PaintItemSpecialEffects(mGraphics g, Item item, int x, int y, int w, int h)
	{
		if (item == null || item.itemOption == null)
		{
			return;
		...
		// Nếu muốn vẫn giữ hiệu ứng ở chỗ khác (VD: ngoài map)
		foreach (var option in item.itemOption)
		{
			if (option == null || option.optionTemplate == null)
				continue;

			paintOptItemInventory(g, option.optionTemplate.id, option.param, x, y, w, h, item);
			paintOptSlotItem(g, option.optionTemplate.id, option.param, x, y, w, h);
		}
	}
	...
	private void doFireShop()
	{
		...
		if (currentTabIndex < currentTabName.Length - ((GameCanvas.panel2 == null) ? 1 : 0) && type != 17)
		{
			...
			if (currItem != null)
			{
				...
				else if (currItem.buyCoin == 0 && currItem.buyGold == 0 && currItem.buyRuby == 0)
				{
				...
				else if (typeShop != 2)
				{
					...
					if (currItem.buyRuby > 0)
					{
						myVector.addElement(new Command(mResources.buy_with + "\n" + Res.formatNumber2(currItem.buyRuby) + "\n" + mResources.RUBY, this, 3001, currItem));
					}
					myVector.addElement(new Command(ModFunc.strAutoBuy, this, 3006, currItem));
				}
				else
	...
	private bool canUseAutoItem(Item item)
	{
		...
		if (type == Item.TYPE_SACH || type == Item.TYPE_NHIEMVU || type == Item.TYPE_GOLD || type == Item.TYPE_DIAMOND || type == Item.TYPE_NGOCRONG || type == 22)
		{
			return false;
		}
		if (item.template.id == 193 || item.template.id == 194 || item.template.id == 570)
		{
			return false;
		}
		string text = (item.template.name ?? string.Empty).ToLowerInvariant();
		string text2 = new string(new char[7] { 'h', (char)7897, 'p', ' ', 'q', 'u', (char)224 });
		string text3 = new string(new char[5] { 'r', (char)432, (char)417, 'n', 'g' });
		string text4 = new string(new char[7] { 't', (char)250, 'i', ' ', 'q', 'u', (char)224 });
		string text5 = new string(new char[8] { 'n', 'h', 'i', (char)7879, 'm', ' ', 'v', (char)7909 });
		string text6 = new string(new char[7] { 'c', 'a', 'p', 's', 'u', 'l', 'e' });
		if ((item.template.description ?? string.Empty).ToLowerInvariant().Contains(text5))
		{
			return false;
		}
		return type == Item.TYPE_DAUTHAN || type == 29 || type == 33 || item.template.id == 380 || text.Contains(text6) || text.Contains(text2) || text.Contains(text3) || text.Contains(text4);
	}

	private bool isPetEquipment(Item item)
	{
		return item != null && item.template != null && item.template.type != 27 && item.isTypeBody();
	}

	private void addInventoryItemCommands(MyVector commands, Item item)
	{
		if (GameCanvas.panel.type == 12)
		{
			commands.addElement(new Command(mResources.use_for_combine, this, 6000, item));
		}
		else if (GameCanvas.panel.type == 13)
		{
			commands.addElement(new Command(mResources.use_for_trade, this, 7000, item));
		}
		else if (canUseAutoItem(item))
		{
			commands.addElement(new Command(mResources.USE, this, 2001, item));
		}
		else if (isPetEquipment(item))
		{
			commands.addElement(new Command(mResources.USE, this, 2000, item));
			if (Char.myCharz().havePet)
			{
				commands.addElement(new Command(mResources.MOVEFORPET, this, 2005, item));
			}
			if (Char.myCharz().havePet2)
			{
				commands.addElement(new Command(ModFunc.strUseForPet2, this, 2007, item));
			}
		}
		else
		{
	...
	private void addAutoItemCommand(MyVector commands, Item item)
	{
		if (ModFunc.GI().listItemAuto.Exists((ItemAuto autoItem) => autoItem.id == item.template.id && autoItem.iconID == item.template.iconID))
		{
			commands.addElement(new Command(ModFunc.strRemoveAutoItem, ModFunc.GI(), 501, item));
		}
		else
		{
			commands.addElement(new Command(ModFunc.strAddAutoItem, ModFunc.GI(), 500, item));
		}
	}
	...
	private void doFireInventory()
	{
		...
		else
		{
			...
			if (isnewInventory)
			{
				...
				{
					flag = true;
					addInventoryItemCommands(myVector, currItem);
				}
			}
			...
			if (currItem != null)
			{
				...
				if (GameCanvas.panel.type != 12 && GameCanvas.panel.type != 13)
				{
					if (position == 0)
					{
						...
						if (flag && canUseAutoItem(currItem))
						{
							addAutoItemCommand(myVector, currItem);
						}
					}
					if (position == 1)
					{
	...
	public void perform(int idAction, object p)
	{
		switch (idAction)
		{
			...
			case 12002:
				...
				GameScr.info1.addInfo("\u0110ang ch\u1ec9nh s\u1eeda v\u1ecb tr\u00ed n\u00fat (L\u01b0u/Reset \u1edf tr\u00ean)", 0);
				break;
		}
		if (idAction == 9999)
		{
