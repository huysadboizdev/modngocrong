...
public class Session_ME : ISession
{
	...
	public void doConnect(string host, int port)
	{
        ...
        connecting = false;
        key = null;
        doSendMessage(ClientAccessHandshake.CreateMessage());
	}
...
internal static class ClientAccessHandshake
{
    private const int ProtocolVersion = 1;
    private const string BuildId = "NROVEGETA-2026.08.15";
    private const string AccessSecret = "DTmdah+xHZqwP1oVfeOeLAKzspQZkHXPoXjL1bg7s7E=";
    ...
    {
        long unixTime = (long)(DateTime.UtcNow - UnixEpoch).TotalSeconds;
        if (unixTime > int.MaxValue)
        {
            throw new InvalidOperationException("Client access timestamp is out of range.");
        }
        ...
        string canonical = ProtocolVersion.ToString(CultureInfo.InvariantCulture)
            + "\n" + BuildId
            + "\n" + timestamp.ToString(CultureInfo.InvariantCulture)
            + "\n" + nonceHex;

        byte[] signature;
        byte[] secretBytes = Encoding.UTF8.GetBytes(AccessSecret);
        ...
        message.writer().writeByte(ProtocolVersion);
        message.writer().writeUTF(BuildId);
        message.writer().writeInt(timestamp);
        message.writer().writeByte(nonce.Length);
        message.writer().write(ToSByteArray(nonce));
        message.writer().writeByte(signature.Length);
        message.writer().write(ToSByteArray(signature));
        return message;
    }
    ...
    private static sbyte[] ToSByteArray(byte[] value)
    {
        sbyte[] result = new sbyte[value.Length];
        for (int i = 0; i < value.Length; i++)
        {
            result[i] = unchecked((sbyte)value[i]);
        }
        return result;
    }
}
