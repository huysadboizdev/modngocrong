    ...
    public void onMessage(Message msg)
    {
        ...
        try
        {
                ...
                case -95:
                    {
                        ...
                        if (type == 5)
                        {
                            ...
                            double num148 = msg.reader().readDouble();
                ...
                case -74:
                    {
                        ...
                        // Nếu chưa tải tài nguyên từ server
                        ...
                        switch (b38)
                        {
                            case 0:
                                {
                                    int resVersion = msg.reader().readInt();

                                    string text3 = Rms.loadRMSString("ResVersion");
                                    int localVersion = (text3 == null || text3 == string.Empty) ? -1 : int.Parse(text3);

                                    if (Session_ME.gI().isCompareIPConnect())
                                    {
                                        if (localVersion == -1 || localVersion != resVersion)
                                        {
                            ...
                            case 2:
                                {
                                    try
                                    {
                                        ...
                                        string[] parts = Res.split(original, "/", 0);
                                        string filename = "x" + mGraphics.zoomLevel + parts[parts.Length - 1];
                                        int length = msg.reader().readInt();
                                        sbyte[] data = new sbyte[length];
                                        msg.reader().read(ref data, 0, length);
                            ...
                            case 3:
                                {
                                    ...
                                    ModFunc.InitMusic();
                                    isLoadingData = false;
                                    int version = msg.reader().readInt();
                                    Rms.saveRMSString("ResVersion", version.ToString());
                                    Service.gI().getResource(3, null);
                                    GameCanvas.endDlg();
                                    SmallImage.loadBigRMS();
                                    mSystem.gcc();
                ...
                case -44:
                    {
                        ...
                        {
                            string shopTabLabel = msg.reader().readUTF();
                            bool isRubyShopTab = !string.IsNullOrEmpty(mResources.RUBY)
                                && shopTabLabel.ToLower().Contains(mResources.RUBY.ToLower());
                            string[] array5 = Res.split(shopTabLabel, "\n", 0);
                            if (b32 == 2)
                            {
                            ...
                            for (int num69 = 0; num69 < num68; num69++)
                            {
                                ...
                                else if (b32 == 0)
                                {
                                    ...
                                    if (isRubyShopTab)
                                    {
                                        Char.myCharz().arrItemShop[num67][num69].buyRuby = Char.myCharz().arrItemShop[num67][num69].buyGold;
                                        Char.myCharz().arrItemShop[num67][num69].buyGold = 0;
                                    }
                                }
                                else if (b32 == 1)
                                {
            ...
            switch (msg.command)
            {
                ...
                case -3: // case show tiem nang , suc manh
                    {
                        sbyte type = msg.reader().readByte();
                        long param = 0;
                        if (ModFunc.isReadInt)
                            ...
                            param = (long)msg.reader().readLong();
                        else
                            param = msg.reader().readLong();
                        if (type == 0)
                        {
                ...
                case 0:
                    {
                        ...
                        try
                        {
                            sbyte canNapTienFlag = msg.reader().readByte();
                            Panel.CanNapTien = canNapTienFlag == 1;
                            sbyte adminLinkFlag = msg.reader().readByte();
                            Rms.saveRMSInt("AdminLink", adminLinkFlag);
                        }
                        catch (Exception)
                        {
                        }
                ...
                case 1:
                    {
                        try
                        {
                            ...
                            if (dataType == 0)
                                ModFunc.SetDataType(ModFunc.DataType.Int);
                            else
                                ModFunc.SetDataType(ModFunc.DataType.Long);

                            Rms.saveRMSInt("dataType", (int)ModFunc.currentDataType);
                        }
                        catch (Exception)
                        { }
                        break;
