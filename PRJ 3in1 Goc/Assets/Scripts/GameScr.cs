...
public class GameScr : mScreen, IChatable
{
	...
	private void paintImageBar(mGraphics g, bool isLeft, Char c)
	{
		...
		if (c.charID == Char.myCharz().charID)
		{
			...
			// {
			// 	double hpLong = (long)(dHP * (double)hpBarW / c.cHPFull);
			// 	double mpLong = (long)(dMP * (double)mpBarW / c.cMPFull);
			// 	double currHPLong = (long)(c.cHP * (double)hpBarW / c.cHPFull);
	...
	private void paintSelectedSkill(mGraphics g)
	{
		...
		else
		{
			...
			{
				int offsetYWhite = -5; //điều chỉnh phần nền trắng (âm = nâng lên, dương = hạ xuống)
				...
				if (gamePad.isSmallGamePad)
				{
					if (isAnalog != 1)
					{
						// NỀN 
						g.setColor(9670800);
						g.fillRect(xHP + 9, yHP + 12, 22, 20);
						g.setColor(16777215);
						g.fillRect(xHP + 9, yHP + 12 + ((num3 != 0) ? (20 - num3) : 0) + offsetYWhite, 22, (num3 == 0) ? 20 : num3);

						// ICON & SỐ ĐẬU
						g.drawImage((mScreen.keyTouch != 10) ? imgHP1 : imgHP2, xHP + 20, yHP + 20, mGraphics.HCENTER | mGraphics.VCENTER);
					...
					else if (isAnalog == 1)
					{
						...
						if (num2 < 10000)
						{
							g.setColor(2721889);
							num3 = (int)(num2 * 20 / 10000);
							g.fillRect(xSkill + xHP + 3, yHP + 3 + num3 + offsetYWhite, 20, 20 - num3);
						}
					}
				}
				else if (isAnalog != 1)
				{
					// ==== NỀN XÁM VÀ NỀN TRẮNG (PC MODE) ====
					g.setColor(9670800);
					g.fillRect(xHP + 9, yHP + 12 + offsetYWhite, 22, 20);
					g.setColor(16777215);
					g.fillRect(xHP + 9, yHP + 10 + ((num3 != 0) ? (20 - num3) : 0) + offsetYWhite, 22, (num3 == 0) ? 20 : num3);

					// ==== ICON & SỐ ĐẬU ====
					g.drawImage((mScreen.keyTouch != 10) ? imgHP3 : imgHP4, xHP + 20, yHP + 20 + offsetYWhite, mGraphics.HCENTER | mGraphics.VCENTER);

					// Dịch số đậu sang trái (-3) và xuống (+2)
					mFont.tahoma_7_green2.drawString(
						g,
						string.Empty + hpPotion,
						xHP + 20 + 1,    // dịch sang trái
						yHP + 11 + offsetYWhite + 3,  // hạ xuống
						2
					);
				}
				else
				{
					g.setColor(9670800);
					g.fillRect(xHP + 10, yHP + 12 + offsetYWhite, 20, 18);
					g.setColor(16777215);
					g.fillRect(xHP + 9, yHP + 15 + ((num3 != 0) ? (20 - num3) : 0) + offsetYWhite, 20, (num3 == 0) ? 18 : num3);
