...
public class ChatPopup : Effect2, IActionListener
{
	...
	public override void paint(mGraphics g)
	{
		...
		// if (maxStarSlot > 4)
		// {
		// ===== Vẽ sao: mỗi hàng tối đa 6 sao =====
		int spacing = 20;                 // khoảng cách giữa các sao
		int starsPerRow = 6;              // số sao tối đa trên một hàng
		int centerX = num + num3 / 2;     // trung tâm popup
		int baseY = num2 + num4 - 20;     // vị trí Y hàng đầu tiên

		int totalRows = maxStarSlot / starsPerRow;
		if (maxStarSlot % starsPerRow != 0)
		{
		...
		{
			int startIndex = row * starsPerRow;
			int remainingStars = maxStarSlot - startIndex;
			int countInRow = remainingStars >= starsPerRow ? starsPerRow : remainingStars;

			// Căn giữa theo chiều ngang
			int totalWidth = countInRow * spacing;
			int startX = centerX - totalWidth / 2 + 5;

			int y = baseY + row * 10; // mỗi hàng cách nhau 10px

			// ===== Vẽ khung sao =====
			for (int i = 0; i < countInRow; i++)
			{
				int x = startX + i * spacing;
				g.drawImage(Panel.imgMaxStar, x, y, 3);
			}
			...
			for (int i = 0; i < countInRow; i++)
			{
				...
				if (currentIndex >= starSlot)
				{
					break; // vẽ đủ số sao thực
				}

				int x = startX + i * spacing;
				g.drawImage(Panel.imgStar, x, y, 3);
			}
		}
